...
const Activities = () => {
  ...
  const [coTransitionDirection, setCoTransitionDirection] = useState<'left' | 'right' | null>(null)
  ...
  const [exTransitionDirection, setExTransitionDirection] = useState<'left' | 'right' | null>(null)
  ...
  const [ovTransitionDirection, setOvTransitionDirection] = useState<'left' | 'right' | null>(null)
  ...
  const coCurricularActivities = [
    {
      ...
      caption: "Holding a poster of Jumpstart 2024, designed by my committee, the creatives committee",
    },
    {
    ...
    },
    {
      ...
      caption: "With the PUP Social Studies Guild during our participation in Ilog Pasiglahin",
    },
    {
      ...
      caption: "Received my President's Lister award during the COED Awards: Gawad Hiraya",
    },
    {
      ...
      caption: "Proudly holding my President's Lister award during the PUP SSG 10th Anniversary and Recognition Day",
    },
    {
    ...
    },
    {
      ...
      caption: "Certificate of completion from UPOU's course: Introduction to Technology for Teaching and Learning",
    },
    {
      ...
      caption: "Group photo with participants from our outreach program at Barangay 592, held in July 2023",
    },
    {
      ...
      caption: "Engaged in an activity during the Earth Day Celebration organized by 360Pilipinas",
    },
    {
      ...
      caption: "Logo of COED Jumpstart 2022, the annual college orientation program",
    },
  ]
  ...
  const extraCurricularActivities = [
    {
      ...
      caption: "Featured in the pubmat as part of The Limestone's Editorial Board for the current publication year",
    ...
    },
    {
      ...
      caption: "Introduced through a pubmat as one of the newly appointed officers of BEST Society for A.Y. 2024-2025",
    ...
    },
    {
      ...
      caption: "Included in the official pubmat release for the newly elected officers of PUP Social Studies Guild",
    ...
    },
    {
      ...
      caption: "Highlighted in a pubmat after being appointed as a Junior Council Officer under the PUP COED Student Council",
  ...
  const overallActivitiesEvent = [
    {
      ...
      caption: "Photo with my co-tutor and YDW mentor alongside our DepEd focal person during our deployment at F. Amorsolo Elementary School for Tara, Basa! tutoring program",
    },
    {
      ...
      caption: "Captured moment with the entire BEST Society during our GTKY: Getting to Know You gathering",
    },
    {
    ...
    },
    {
      ...
      caption: "Taking a photo of a graduating COED student during their photoshoot for the upcoming pinning ceremony",
    },
    {
      ...
      caption: "Facilitating the attendance of participants during the Social Science Quiz Bee",
    },
    {
      ...
      caption: "Photo with NSTP, FBTO, LISSO, and BEST Society as core organizers of the COEDYSSEY Leadership Seminar",
    },
    {
      ...
      caption: "At the Balik Sinta event, stationed at our booth as part of BEST Society, one of the partner organizations",
    },
    {
      ...
      caption: "Group photo with participants of the Leadership Empowerment Talks",
    },
    {
      title: "PUP Rainbow",
      ...
      caption: "With fellow BEST Society members during our participation as a partner organization in PUP Rainbow 2024",
    },
  ]
  ...
  const handleCoDragEnd = () => {
    if (!coIsDragging || coDragStartX.current === null || coDragCurrentX.current === null || coDragStartTime.current === null) {
    ...
    if (distance < -threshold || (distance < 0 && velocity > velocityThreshold)) {
      setCoCurricularPageAnimated(Math.min(Math.ceil(coCurricularActivities.length / itemsPerPage), coCurricularPage + 1))
      navigated = true
  ...
  const handleExDragEnd = () => {
    if (!exIsDragging || exDragStartX.current === null || exDragCurrentX.current === null || exDragStartTime.current === null) {
    ...
    let navigated = false
    if (distance < -threshold || (distance < 0 && velocity > velocityThreshold)) {
      setExtraCurricularPageAnimated(Math.min(Math.ceil(extraCurricularActivities.length / itemsPerPage), extraCurricularPage + 1))
      navigated = true
    } else if (distance > threshold || (distance > 0 && velocity > velocityThreshold)) {
  ...
  const handleOvDragEnd = () => {
    if (!ovIsDragging || ovDragStartX.current === null || ovDragCurrentX.current === null || ovDragStartTime.current === null) {
    ...
    let navigated = false
    if (distance < -threshold || (distance < 0 && velocity > velocityThreshold)) {
      setOverallPageAnimated(Math.min(Math.ceil(overallActivitiesEvent.length / itemsPerPage), overallPage + 1))
      navigated = true
    } else if (distance > threshold || (distance > 0 && velocity > velocityThreshold)) {
        ...
        >
          ...
          {Math.ceil(coCurricularActivities.length / itemsPerPage) > 1 && (
            <div className="flex items-center justify-center gap-4 mt-auto">
              {/* Previous Button */}
              <button
                onClick={() => setCoCurricularPageAnimated(Math.max(1, coCurricularPage - 1))}
              ...
              <div className="flex items-center gap-2">
                {(() => {
                  const total = Math.ceil(coCurricularActivities.length / itemsPerPage)
                  const dots = []
                  if (coCurricularPage > 1) {
                    dots.push(
                      <button
                        key={coCurricularPage - 1}
                        onClick={() => setCoCurricularPageAnimated(coCurricularPage - 1)}
              ...
              {/* Next Button */}
              <button
                onClick={() => setCoCurricularPageAnimated(Math.min(Math.ceil(coCurricularActivities.length / itemsPerPage), coCurricularPage + 1))}
                disabled={coCurricularPage === Math.ceil(coCurricularActivities.length / itemsPerPage)}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-300 ${
          ...
          <div className="text-center mt-4">
            <span className="text-sm text-gray-400">
              Page {coCurricularPage} of {Math.ceil(coCurricularActivities.length / itemsPerPage)} • {coCurricularActivities.length} activities
            </span>
          </div>
        </div>
      </div>
          ...
          >
            ...
            {Math.ceil(extraCurricularActivities.length / itemsPerPage) > 1 && (
              <div className="flex items-center justify-center gap-4 mt-auto">
                {/* Previous Button */}
                <button
                  onClick={() => setExtraCurricularPageAnimated(Math.max(1, extraCurricularPage - 1))}
                ...
                <div className="flex items-center gap-2">
                  {(() => {
                    const total = Math.ceil(extraCurricularActivities.length / itemsPerPage)
                    const dots = []
                    if (extraCurricularPage > 1) {
                      dots.push(
                        <button
                          key={extraCurricularPage - 1}
                          onClick={() => setExtraCurricularPageAnimated(extraCurricularPage - 1)}
                ...
                {/* Next Button */}
                <button
                  onClick={() => setExtraCurricularPageAnimated(Math.min(Math.ceil(extraCurricularActivities.length / itemsPerPage), extraCurricularPage + 1))}
                  disabled={extraCurricularPage === Math.ceil(extraCurricularActivities.length / itemsPerPage)}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-300 ${
            ...
            <div className="text-center mt-4">
              <span className="text-sm text-gray-400">
                Page {extraCurricularPage} of {Math.ceil(extraCurricularActivities.length / itemsPerPage)} • {extraCurricularActivities.length} activities
              </span>
            </div>
          </div>
        </div>
          ...
          >
            ...
            {Math.ceil(overallActivitiesEvent.length / itemsPerPage) > 1 && (
              <div className="flex items-center justify-center gap-4 mt-auto">
                {/* Previous Button */}
                <button
                  onClick={() => setOverallPageAnimated(Math.max(1, overallPage - 1))}
                ...
                <div className="flex items-center gap-2">
                  {(() => {
                    const total = Math.ceil(overallActivitiesEvent.length / itemsPerPage)
                    const dots = []
                    if (overallPage > 1) {
                      dots.push(
                        <button
                          key={overallPage - 1}
                          onClick={() => setOverallPageAnimated(overallPage - 1)}
                ...
                {/* Next Button */}
                <button
                  onClick={() => setOverallPageAnimated(Math.min(Math.ceil(overallActivitiesEvent.length / itemsPerPage), overallPage + 1))}
                  disabled={overallPage === Math.ceil(overallActivitiesEvent.length / itemsPerPage)}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-300 ${
            ...
            <div className="text-center mt-4">
              <span className="text-sm text-gray-400">
                Page {overallPage} of {Math.ceil(overallActivitiesEvent.length / itemsPerPage)} • {overallActivitiesEvent.length} activities
              </span>
            </div>
          </div>
        </div>
